...
The compiler error shows that the shortcut no longer applies within the `customer` module: કમ્પાઇલરની ભૂલ દર્શાવે છે કે આ ટૂંકા માર્ગ હવે `customer` મોડ્યુલમાં લાગુ પડતો નથી.
...
Adding `rand` as a dependency in `Cargo.toml` `Cargo.toml` માં `rand` ને આધાર તરીકે ઉમેરવાથી કાર્ગોને `rand` પેકેજ અને crates.io પરથી તેના આધારિત વસ્તુઓ ડાઉનલોડ કરવા અને આપણાં પ્રોજેક્ટ
...
In bigger programs, bringing many items into scope from the same crate or module using nested paths can reduce the number of separate `use` statements needed by a lot! વિશાળ કાર્યક્રમોમાં, સમાન crate અથવા module માંથી અનેક વસ્તુઓને સ્કોપમાં
