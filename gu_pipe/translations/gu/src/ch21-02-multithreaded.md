...
We switched from `if` to `match` now that we have three cases. અમે ત્રણ પરિસ્થિતિઓ હોવાથી, `if` થી `match` પર સ્વિચ કર્યું છે. આપણે `request_line` ના સ્લાઇસ સાથે પેટર્ન-મેચ કરવા માટે સ્પષ્ટપણે મેચ કરવાની જરૂર છે; `match` આપોઆપ રેફરન્સિંગ અને ડિરેફરન્સિંગ કરતું
...
We’ve changed the name of the field on `ThreadPool` from `threads` to `workers` because it’s now holding `Worker` instances instead of `JoinHandle<()>` instances. આપણે `ThreadPool` પર ક્ષેત્રનું નામ `threads` થી બદલીને `workers` કર્યું છે, કારણ કે હવે તે `JoinHandle<()>` ઉદાહરણોને બદલે `Worker` ઉદાહરણો ધરાવે છે. આપણે `for` લૂપમાં કાઉન્ટરનો ઉપયોગ `Worker::new` માટે દલીલ તરીકે કરીએ છીએ, અને દરેક નવા `Worker`
