...
The `largest_i32` Function `largest_i32` વિધેય એListing 10-3 માં આપણે જે કાઢ્યું તે છે, જે સ્લાઇસમાં સૌથી મોટું `i32` શોધે છે. `largest_char` વિધેય સ્લાઇસમાં સૌથી મોટું `char` શોધે છે. બંને વિધેયોના મુખ્ય ભાગ સમાન કોડ ધરાવે છે, તેથી ચાલો આપણે એક જ વિધેયમાં સામાન્ય પ્રકાર પરિમાણ (generic type parameter) દાખલ કરીને આ બિનજરૂરીયાત દૂર કરીએ. એક
...
We read this definition as “The function `largest` is generic over some type `T` .” આ વ્યાખ્યાને આપણે આ રીતે સમજીએ છીએ: "કાર્ય `largest` અમુક પ્રકાર `T` પર સામાન્ય છે." આ કાર્યમાં એક પરિમાણ નામ `list` છે, જે પ્રકાર `T` ના મૂલ્યોની સ્લાઇસ છે. `largest` કાર્ય સમાન પ્રકાર `T` ના મૂલ્યનો સંદર્ભ આપશે. Listing
...
`main` માં, આપણે એક `Point` વ્યાખ્યાયિત કર્યો છે જેમાં `x` માટે `i32` (મૂલ્ય `5` સાથે) અને `y` માટે `f64` (મૂલ્ય `10.4` સાથે) છે. `p2` ચલ એ `Point` struct છે જેમાં `x` માટે સ્ટ્રિંગ સ્લાઇસ (મૂલ્ય `"Hello"` સાથે) અને `y` માટે `char` (મૂલ્ય `c` સાથે) છે. `p1` પર `mixup` ને `p2` દલીલ સાથે બોલાવવાથી આપણને `p3` મળે છે, જેમાં `x` માટે `i32` હશે કારણ કે `x`, `p1` માંથી આવ્યું છે. `p3` ચલમાં `y` માટે `char` હશે કારણ કે `y`, `p2` માંથી આવ્યું છે. `println!` મેક્રો કૉલ `p3.x = 5, p3.y = c` પ્રિન્ટ કરશે.
