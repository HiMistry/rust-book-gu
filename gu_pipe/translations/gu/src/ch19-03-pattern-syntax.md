...
આ કોડ `a` અને `b` નાં ચલ બનાવે છે જે `p` સ્ટ્રક્ચરના `x` અને `y` ક્ષેત્રોના મૂલ્યો સાથે મેળ ખાય છે. આ ઉદાહરણ દર્શાવે છે કે પેટર્નમાં ચલોનાં નામ સ્ટ્રક્ચરના ક્ષેત્રનાં નામો સાથે મેળ ખાતા હોવા જરૂરી નથી. જોકે, કયો ચલ કયા ક્ષેત્રમાંથી આવ્યો છે તે યાદ રાખવામાં સરળતા રહે તે માટે ઘણીવાર ચલોનાં નામોને ક્ષેત્રોનાં નામો સાથે મેળવવામાં આવે છે. આ સામાન્ય પ્રયોગને લીધે અને `let Point { x: x, y: y } = p;` લખવાથી ઘણો પુનરાવર્તન થાય છે, Rust પેટર્ન માટે એક ટૂંકું રૂપ ધરાવે છે જે સ્ટ્રક્ચર ક્ષેત્રો સાથે મેળ ખાય છે: તમારે ફક્ત સ્ટ્રક્ચર ક્ષેત્રનું નામ સૂચિબદ્ધ કરવાની જરૂર છે, અને પેટર્નમાંથી બનાવેલા ચલોનાં નામો સમાન હશે. યાદી 19-13 યાદી 19-12 માં રહેલા કોડની જેમ જ વર્તે છે, પરંતુ `let` પેટર્નમાં બનાવેલા ચલ `x` અને `y` છે, `a` અને `b` નહીં.
...
આ કોડ `x` અને `y` નામના ચલ બનાવે છે જે `p` ચલના `x` અને `y` ક્ષેત્રો સાથે મેળ ખાય છે. પરિણામ એ આવે છે કે `x` અને `y` ચલમાં `p` સ્ટ્રક્ચરની કિંમતો હોય છે.
...
પહેલો ભાગ કોઈપણ બિંદુ સાથે મેળ ખાય છે જે `x` અક્ષ પર આવેલું હોય, જો `y` ક્ષેત્રનું મૂલ્ય શાબ્દિક `0` સાથે મેળ ખાતું હોય તો. પેટર્ન હજી પણ એક `x` ચલ બનાવે છે જેનો ઉપયોગ આપણે આ ભાગ માટે કોડમાં કરી શકીએ છીએ. એ
...
આ કોડ `Change color to red 0, green 160, and blue 255` પ્રિન્ટ કરશે. જુદા જુદા આર્મ્સના કોડને ચલાવવા માટે `msg` નું મૂલ્ય બદલીને પ્રયત્ન કરો. ડેટા વગરના enum variants માટે, જેમ કે `Message::Quit`, અમે મૂલ્યને વધુ વિઘટિત કરી શકતા નથી. અમે ફક્ત સાચું `Message::Quit` મૂલ્ય મેળવી શકીએ છીએ, અને તે પેટર્નમાં કોઈ ચલ નથી.
...
આ કોડ `3` મૂલ્યને પ્રથમ દલીલ તરીકે સંપૂર્ણપણે અવગણશે અને `This code only uses the y parameter: 4` છાપશે. મોટાભાગના કિસ્સાઓમાં જ્યારે તમારે
...
અહીં, આપણને ચિંતાજનક સંદેશો મળે છે કે ચલ `y` નો ઉપયોગ કરવામાં આવ્યો નથી, પરંતુ આપણને `_x` ના ઉપયોગ ન થવા અંગે
...
આપણે એક ભૂલ મેળવીશું કારણ કે `s` મૂલ્ય હજી પણ `_s` માં ખસેડવામાં આવશે, જે આપણને `s` ફરીથી વાપરતા અટકાવશે. જોકે, ફક્ત અન્ડરસ્કોરનો ઉપયોગ કરવાથી ક્યારેય કોઈ મૂલ્ય સાથે જોડાતું નથી. સૂચિ ૧૯-૨૨ કોઈપણ ભૂલો વગર સંકલિત થશે કારણ કે `s`  `_` માં ખસેડવામાં આવતું નથી.
...
We list the `x` value and then just include the `..` pattern. This is quicker than having to list `y: _` and `z: _`, particularly when we’re working with structs that have lots of fields in situations where only one or two fields are relevant. આપણે `x` મૂલ્ય દર્શાવેલું અને પછી માત્ર `..` પેટર્ન ઉમેર્યું છે. આ `y: _` અને `z: _` સૂચિબદ્ધ કરવાની જરૂરિયાત કરતાં વધુ ઝડપી છે, ખાસ કરીને જ્યારે આપણે એવા structures સાથે કામ કરી રહ્યા હોઈએ જેમાં ઘણા ક્ષેત્રો હોય અને માત્ર એક કે બે જ ક્ષેત્રો
