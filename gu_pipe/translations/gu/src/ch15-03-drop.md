...
Dropping CustomSmartPointer with data `some data`! `CustomSmartPointer created` અને `CustomSmartPointer dropped before the end of main` લખાણો વચ્ચે `Dropping CustomSmartPointer with data \`some data\`!` છાપવામાં આવે છે, જે દર્શાવે છે કે `drop` પદ્ધતિનો કોડ `c` ને તે સમયે છોડવા
